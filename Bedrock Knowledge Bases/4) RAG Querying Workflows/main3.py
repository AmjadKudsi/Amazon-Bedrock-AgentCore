# implement the full retrieve_and_generate API call, which combines document retrieval with intelligent response generation
...
try:
    ...
    for query in rag_queries:
        ...
        # TODO: Implement the complete retrieve_and_generate call with:
        # - input parameter with the query text
        # - retrieveAndGenerateConfiguration with type "KNOWLEDGE_BASE"
        # - knowledgeBaseConfiguration containing:
        #   - knowledgeBaseId (use KNOWLEDGE_BASE_ID variable)
        #   - modelArn (construct ARN using REGION_NAME and GENERATION_MODEL_ID)
        #   - retrievalConfiguration with vectorSearchConfiguration (numberOfResults: 3)
        #   - generationConfiguration with textInferenceConfig (maxTokens: 500, temperature: 0.1)
        ...
        if citations:
            ...
            for citation in citations:
                for ref in citation.get("retrievedReferences", []):
                    ...
                    source_key = metadata.get("x-amz-bedrock-kb-source-uri", metadata.get("filename", "Unknown"))
                    print(f"  - {source_key}")
        else:
